...
function updateLiveVoices() {
  ...
  harpVoices.forEach((voice, ringIndex) => {
    const holes = getRingHoles(ringIndex);
    const freq = freqFromHolesAndRpm(holes, state.rpm);
    if (freq < 20) return;
    voice.osc.frequency.setTargetAtTime(freq, now, 0.012);
    try {
      voice.osc.setPeriodicWave(buildPeriodicWave(getPulseGeometry(holes).duty));
    } catch {
      /* square fallback */
    }
  });
...
function resetAllDefaults() {
  ...
  state.variacSensitivity = DEFAULT_SETTINGS.variacSensitivity;
  setVariacSensitivity(state.variacSensitivity);
...
async function beginRingPlay(ringIndex, selectUi = true) {
  if (ringIndex < 0) return;
  if (!harpVoices.has(ringIndex)) {
    await startHarpVoice(ringIndex, { latched: true });
  }
  if (selectUi) selectRing(ringIndex, true);
  setHarpStatus(ringIndex);
...
function harpPointerDown(ringIndex) {
  ...
  if (state.harpPlayMode === "once") {
    void beginRingPlay(ringIndex, true);
  }
}
...
function togglePlayMode() {
  state.harpPlayMode = state.harpPlayMode === "sustain" ? "once" : "sustain";
  const radio = document.querySelector(
    `input[name="harpPlayMode"][value="${state.harpPlayMode}"]`
  );
  if (radio) radio.checked = true;
  if (state.harpPlayMode === "once") releaseAllHarpVoices();
...
const VARIAC_STEP_MIN = 0.25;
const VARIAC_STEP_MAX = 12;

function getVariacWheelStep(sensitivity = state.variacSensitivity) {
  const t = Math.max(0, Math.min(100, sensitivity)) / 100;
  return VARIAC_STEP_MIN * Math.pow(VARIAC_STEP_MAX / VARIAC_STEP_MIN, t);
}

function formatVariacStep(step) {
  if (step < 1) return step.toFixed(2);
  if (step < 10) return step.toFixed(1);
  return String(Math.round(step));
}

function variacSensBand(sensitivity = state.variacSensitivity) {
  if (sensitivity < 28) return "fine";
  if (sensitivity > 72) return "fast";
  return "mid";
}

function getEffectiveVariacStep(shiftKey = false) {
  const step = getVariacWheelStep();
  return shiftKey ? step / 2 : step;
}

function updateVariacSensUI(shiftPreview = false) {
  const step = getVariacWheelStep();
  const stepFine = step / 2;
  const band = variacSensBand();
  const label = $("variacSensLabel");
  const hint = $("variacStepHint");
  const slider = $("variacSensitivity");
  const zone = $("variacPanel");
  if (slider) slider.value = state.variacSensitivity;
  if (label) {
    label.textContent = t(`controls.variacSens${band === "fine" ? "Fine" : band === "fast" ? "Fast" : "Mid"}`);
  }
  if (hint) {
    const activeStep = shiftPreview ? stepFine : step;
    hint.textContent = shiftPreview
      ? t("controls.variacStepHintActive", { step: formatVariacStep(activeStep) })
      : t("controls.variacStepHint", {
          step: formatVariacStep(step),
          stepFine: formatVariacStep(stepFine),
        });
  }
  if (zone) zone.classList.toggle("shift-fine", shiftPreview);
  document.querySelectorAll(".sens-preset-btn").forEach((btn) => {
    const preset = parseInt(btn.dataset.sens, 10);
    btn.classList.toggle("active", Math.abs(preset - state.variacSensitivity) <= 4);
  });
  const voltRange = $("voltage");
  if (voltRange) {
    const rangeStep = step >= 1 ? 1 : 0.25;
    voltRange.step = String(rangeStep);
  }
}

function setVariacSensitivity(value) {
  state.variacSensitivity = Math.max(0, Math.min(100, Math.round(value)));
  updateVariacSensUI();
}

function adjustVoltageByWheel(deltaY, shiftKey = false) {
  const step = getEffectiveVariacStep(shiftKey);
  const sign = deltaY > 0 ? -1 : 1;
  const v = Math.max(80, Math.min(260, Math.round((state.voltage + sign * step) * 4) / 4));
  state.voltage = v;
  $("voltage").value = v;
  $("voltVal").textContent = Number.isInteger(v) ? v : v.toFixed(1);
  if ($("linkVoltage").checked) {
    const { vMin, vMax, rpmAtVMin, rpmAtVMax } = getInputs();
    syncRpm(voltageToRpm(v, vMin, vMax, rpmAtVMin, rpmAtVMax));
  }
  updateUI();
}

function bindVariacWheel() {
  const zone = $("variacPanel");
  if (!zone) return;
  zone.addEventListener(
    ...
    (e) => {
      ...
      adjustVoltageByWheel(e.deltaY, e.shiftKey);
    },
    { passive: false }
  );
  let shiftHeld = false;
  const refreshShiftPreview = () => {
    ...
    updateVariacSensUI(shiftHeld);
  };

  zone.addEventListener("mouseenter", () => {
    zone.classList.add("wheel-active");
    refreshShiftPreview();
  });
  zone.addEventListener("mouseleave", () => {
    zone.classList.remove("wheel-active");
    updateVariacSensUI(false);
  });
...
function bindGlobalShortcuts() {
  ...
  window.addEventListener("keydown", (e) => {
    ...
    if (n >= 1 && n <= 8) {
      if (e.repeat) return;
      const i = n - 1;
      if (keysHeldRings.has(i)) return;
      keysHeldRings.add(i);
      e.preventDefault();
      void beginRingPlay(i, true);
    }
  });
...
function init() {
  ...
  $("variacSensitivity")?.addEventListener("input", (e) => {
    setVariacSensitivity(parseFloat(e.target.value));
  });

  document.querySelectorAll(".sens-preset-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      setVariacSensitivity(parseInt(btn.dataset.sens, 10));
    });
  });
  ...
  window.onFanLangChange = () => {
    ...
    const wh = document.querySelector('[data-i18n="controls.wheelHint"]');
    if (wh) wh.textContent = t("controls.wheelHint");
  };
